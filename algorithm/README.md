...
<img src="assets/hero.png" width="100%" alt="算法方法图解卡">
...
卡片图均由 **GPT-Image-2** 生成（无源、不可脚本复现）。`scripts/` 仅含少量配套工具：`images_to_pptx.py`（把图片打包成 PPT）、`render_hot100.py`（生成分类总览图）、`make_hero.py`（生成 README 横幅）。
...
├── hot100_images/         # Hot100 题型分类总览图（render_hot100.py 生成）
└── scripts/               # 配套工具脚本（打包 PPT / 总览图 / 横幅）

# PPT 成品见 GitHub Releases
```
